...
def stabilize_robot(env, joint_index):
    ...
    # print("env_dof_states:", env_dof_states[start_index:end_index])
    # print("设置后的 all_dof_states:", all_dof_states[start_index:end_index])
    # print("仿真后的 dof_states:", dof_states[start_index:end_index])

    # env.gym.set_dof_state_tensor(env.sim, gymtorch.unwrap_tensor(fixed_states))
...
def play(args):
    ...
    for i in range(int(env.max_episode_length)):
        ...
        # if i < 50:
        # desired_positions[:, [joint_index]] = 0.0
        # else:
        desired_positions[:, [joint_index]] = amplitude * torch.sin(2 * np.pi * frequency * time_tensor)
        # print(f"desired_positions: {desired_positions[:, [joint_index]]}")
        ...
        # print('joint_positions:',joint_positions[:, 0])
        ...
        # print(desired_positions[:, [joint_index]] - joint_positions)
        # print(torques)
        ...
        if i < stop_state_log:
            logger.log_states(
            {
                f'torque_{args.joint_name}': env.torques[0, joint_index].detach().cpu().numpy(),
                ...
                f'desired_position_{args.joint_name}': actions[:, joint_index].cpu().detach().numpy(),
                f'error_{args.joint_name}': (actions[:, joint_index].detach() - joint_positions[:, 0]).cpu().numpy(),
...
if __name__ == '__main__':
    ...
    args.task='h1'
    args.num_envs = 1
    #env.dof_dict: {'left_ankle_joint': 4, 'left_hip_pitch_joint': 2, 'left_hip_roll_joint': 1, 'left_hip_yaw_joint': 0, 'left_knee_joint': 3, 'right_ankle_joint': 9, 'right_hip_pitch_joint': 7, 'right_hip_roll_joint': 6, 'right_hip_yaw_joint': 5, 'right_knee_joint': 8}
    args.joint_name = 'left_knee_joint'
    args.amplitude = 2  # Amplitude of the sine wave
    args.frequency = 0.1  # Frequency of the sine wave (Hz)
    args.log_dir = '/home/shanhe/unitree_rl_gym/legged_gym/data/h1'
    args.p_gain = 15
    args.d_gain = 2
    # args.d_gain = 2
